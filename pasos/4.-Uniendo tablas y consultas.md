...
```sql
        SELECT 	* 
        FROM credicel_alura_bd.tabla_de_clientes
        WHERE BARRIO IN('Condesa', 'Del Valle','Contader', 'Oblatos')
```

el anterior codigo puede ser cambiado por:
...
![Subconsulta Jugos 1](/imagenes/subconsulta_jugos_1.png)

Se puede agregar esta consulta en un from:
...
![Subconsulta Jugos 2](/imagenes/subconsulta_jugos_2.png)
...
```sql
        ...
        FROM    (SELECT 	ENVASE,
                MAX(PRECIO_DE_LISTA) AS PRECIO_MAXIMO 
                FROM credicel_alura_bd.tabla_de_productos
                GROUP BY ENVASE) AS X
        WHERE   X.PRECIO_MAXIMO >= 10;
```

![Subconsulta Jugos 3](/imagenes/subconsulta_jugos_3.png)


### Vistas VIEWS

- Tabla logica que resulta de una consulta, esta puede ser usada en otra consulta.
- Tiene un costo de procesamiento
- Puede ser usada en otras consultas, comportandose como una subconsulta.
- 
...
![Vistas 1](/imagenes/vistas_1.png)

Usando la vista en una consulta
...
![Vistas 2](/imagenes/vistas_2.png)

Usando la Vista en un JOIN

```sql
        SELECT 	A.NOMBRE_DEL_PRODUCTO,
                A.ENVASE,
                A.PRECIO_DE_LISTA,
                B.PRECIO_MAXIMO 
...
![Vistas 3](/imagenes/vistas_3.png)